...
$profileData = fetchStudentProfile($conn, $student_id);

// Function to fetch student profile data for the logged-in user
function fetchStudentProfile($conn, $student_id) {
    // Query to fetch data from the student_profile table for the logged-in user
    $query = $conn->prepare("SELECT * FROM student_profile WHERE student_id = ?");
