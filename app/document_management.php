    ...
    if (isset($_FILES["file"]) && $_FILES["file"]["error"] == 0) {
        ...
        if (empty($file_err)) {
            ...
            if (move_uploaded_file($file_temp, $file_path)) {
                ...
                $query = $conn->prepare("INSERT INTO documents (student_id, title, description, file_path) VALUES (:student_id, :title, :description, :file_path)");
                $query->execute(array(
...
if (isset($_SESSION["student_id"])) {
    $query = $conn->prepare("SELECT * FROM documents WHERE student_id = :student_id ORDER BY upload_date DESC");
